...
function willYouMarryMe(isPositiveAnswer) {
  ...
  const promise = new Promise((resolve, reject) => {
    if (typeof isPositiveAnswer === 'boolean') {
      if (isPositiveAnswer) { answer = 'Hooray!!! She said "Yes"!'; } else { answer = 'Oh no, she said "No".'; }
      resolve(answer);
    } else {
...
function chainPromises(/* array, action */) {
  throw new Error('Not implemented');
  /* const promise = Promise.allSettled(array).then((result) => {
    const resolvedResults = result.filter((x) => x.status === 'fulfilled');
    const res = resolvedResults.forEach(action);
    return res;
  });
  return promise; */
}
